 ...
 * @更新描述: TODO
 */
public class MyKLineChart extends AppCompatActivity implements OnChartValueSelectedListener

{


    private CombinedChart mChart;
    private List<QuoteHisKUnit> mList;
    ...
    private void initData(List<QuoteHisKUnit> mData) {


        CombinedData data = new CombinedData();

        //        data.setData(generateCandleData(mList));

        data.setData(generateLineData(mList));
        mXAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                float         v             = value % mList.size();
                QuoteHisKUnit quoteHisKUnit = mList.get((int) v);
                String s             = String.valueOf(quoteHisKUnit.mDate);
                return s;
    ...
    private float getSum(Integer a, Integer b) {
        ...
        for (int i = a; i <= b; i++) {
            sum += mList.get(i).mClosePrice;
        }
        return sum;
    }

    private LineData generateLineData(List<QuoteHisKUnit> list) {
...
//        sets.add(setMaLine(10, line10));
    ...
    private CandleData generateCandleData(List<QuoteHisKUnit> mData) {
        ...
        for (int i = 0; i < mData.size(); i++) {
            ...
            al.add(new CandleEntry(i + 1f,
                                   mData.get(i).mMaxPrice,
                                   mData.get(i).mMinPrice,
                                   mData.get(i).mOpenPrice,
                                   mData.get(i).mClosePrice));
        }
    ...
    @Override
    public void onKLine(List<QuoteHisKUnit> list) {
